...
def slide (geology,horizontal,vertical):

    tree_count = 0
    tree_index = 0
    v_down = np.arange(start = 0,stop = len(geology),step = vertical).astype(int)
...
for r, d in zip(toboggan_slope['right'],toboggan_slope['down']):
    tree_count_ls.append(
        slide(geology=in_ls,horizontal=r,vertical=d))

print(np.prod(tree_count_ls))
