...
class BinSeating:
    ...
    def seat_ID(self, b_pass):
        ...
        def bin_divide(char_ls, num, upper_half):
            b_pass_index = 0
            seat_index = []
            seats_ls = np.arange(start=0,stop=num+1,step=1,dtype=int).tolist()
            while b_pass_index <= len(char_ls)-1:
                seat_index = int(np.ceil(np.divide(seats_ls.index(seats_ls[-1]),2)))
        ...
        row_num = bin_divide(char_ls=region,num=self.rows,upper_half='B')
        col_num = bin_divide(char_ls=seat,num=self.cols,upper_half='R')
        seat = np.multiply(row_num, self.cols+1)+col_num


        return {'row' : row_num[0],'col' : col_num[0],'seat': seat[0]}
...
def extract_seat_info(ID_ls, cols):
    ...
    return {'empty_seats' : missing_seats_df,'plane_seats' : all_seats.sort_values(by=['seat']),'own_seat_ID' : own_seat['seat'],}
