...
def eval(code, rows, col, limit=None):
    ...
    def FRONT(interval):
        if np.sum(interval)%2 != 0:
            return [interval[0], interval[1] + 1 - (interval[1] - interval[0] + 1)/2 - 1]
        
        else:
            return [interval[0], interval[1] - (interval[1] - interval[0])/2]
    
    def BACK(interval):
        if np.sum(interval)%2 != 0:
            return [interval[1] + 1 - (interval[1] - interval[0] + 1)/2, interval[1]]
        
        else:
            return [interval[1] - (interval[1] - interval[0])/2, interval[1]]
...
if __name__=="__main__":
    ...
    for idx in range(128):
        planeMap = np.concatenate((planeMap, np.arange(8)), axis=0)
    ...
    mySeatLine = mySeat[0]
    seatID = list()
    for idx, line in enumerate(mySeatLine):
        seatID.append(mySeat[0][idx] * 8 + mySeat[1][idx])
